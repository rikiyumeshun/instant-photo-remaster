...
スマホで撮影またはアップロードした写真を、ブラウザ内で画質補正・高画質化風処理・保存まで行う Next.js アプリです。通常写真はそのまま補正でき、白枠フォトや斜め撮影には台形補正も選べます。

## 使い方

1. 「カメラで撮影」または「画像をアップロード」から JPEG / PNG / WebP / HEIC を選びます。
2. 補正前処理を選びます。
   - **そのまま画質補正**（デフォルト）: 通常写真向け。台形補正せず、読み込んだ画像をそのまま画質補正します。
   - **台形補正してから補正**: 白枠フォトや斜め撮影向け。四隅を合わせて正面化してから画質補正します。
3. 台形補正モードを選んだ場合のみ、自動検出された四隅が白枠の外側に合っているか確認します。ずれている場合は丸いハンドルをドラッグして調整し、「台形補正する」を押します。
4. 補正モードを選びます。
   - **明るく盛る**: 血色ピンク・かわいい方向の強め補正。
   - **白ピンク盛り**: 白肌・白背景・ピンク血色・透明感を目指す寒色ホワイト寄せ。
   - 自然補正: 雰囲気を残して自然に整えます。
   - くっきり補正: コントラストとシャープを強め、SNS向けに整えます。
   - やわらか補正: 淡い色と空気感を残します。
   - レトロ補正: 暖色、浮いた黒、粒状感を少し加えます。
5. 台形補正モードの場合のみ、「白枠込み」または「写真部分のみ」を選べます。写真部分のみの場合は余白スライダーで微調整できます。
6. 「目元くっきり」で目・まつ毛・黒目・白目・目元輪郭を自然に強調できます（0〜3、デフォルト2）。等倍表示で確認すると差が分かりやすいです。
7. Before / After を確認し、「補正後画像を保存」から JPEG で保存します。

### 目元くっきり

MediaPipe Face Landmarker で目周辺のランドマークを検出し、目元だけソフトマスクで局所補正します。

- 自然さ優先のシャープ・マイクロコントラスト・黒目/まつ毛の締め・白目の黄ばみ抑制
- 目の形は変えません（デカ目機能ではありません）
- 強め（3）にするとアイラインが不自然に濃く見える場合があります
- 顔検出に失敗した画像では目元補正のみスキップし、他の補正は続行します

処理順: 色補正（bright / whitePink 等）→ 目元くっきり → 保存
...
- `applyEnhancementPreset()`: 明るさ、コントラスト、ホワイトバランス、彩度、暗部、ハイライト、ノイズ低減、シャープ化。`bright`（明るく盛る）プリセットでは中間調の持ち上げ、白背景の強調、黒レベルの締め、肌色保護付きハイライトロールオフ、肌・中間調向けの自然なピンク寄せも行います。
...
台形補正は「台形補正してから補正」モードでのみ実行されます。四隅は補正前に左上・右上・右下・左下へ並び替えます。四角形が交差している場合、角同士が近すぎる場合、極端に潰れている場合は、分かりやすい日本語エラーを表示して処理を止めます。
...
サーバー側では画像をディスクに保存せず、メモリ上で処理します。

AIサーバーは `AI_PROCESSOR` で切り替え可能です。

- `local_dummy`（デフォルト）: Pillowによる軽量ダミー補正
- `realesrgan`: Real-ESRGAN x2 超解像 PoC（PyTorch CPU/GPU または NCNN Vulkan）

AI高画質化のパイプライン:

```text
corrected canvas → Real-ESRGAN (x2/x4) → applyEnhancementPreset(preset, { brightIntensity, aiPostProcess: true }) → final
```

AI後のプリセット適用では sharpen を約 0.6 倍、denoise を約 0.45 倍に抑え、Real-ESRGAN のシャープと二重にならないようにしています。

Real-ESRGAN PoC の注意:

- 写真は外部AIサーバーへ送信されます
- CPU でも動きますが、320px 級でも数十秒、1920px 級では数分かかる場合があります
- GPU (CUDA) または NCNN Vulkan がある環境の方が現実的です
- PoC では Real-ESRGAN 入力の最大辺を 1920px 程度に制限しています
- モデル未導入・メモリ不足・タイムアウト時は UI に分かりやすいエラーを返します

詳細は `ai-server/README.md` を参照してください。
